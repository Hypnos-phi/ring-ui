...
export const deprecatedPropType = (replacement: string): Validator<unknown> =>
  (props, propName) => {
...
export interface DatePopupBaseProps {
  ...
  hidden: boolean
  parseDateInput: (text: Date | number | string | null | undefined) => Date
  displayFormat: (date: Date) => string
  onComplete: () => void
  onClear?: ((e: React.MouseEvent<HTMLButtonElement>) => void) | undefined
